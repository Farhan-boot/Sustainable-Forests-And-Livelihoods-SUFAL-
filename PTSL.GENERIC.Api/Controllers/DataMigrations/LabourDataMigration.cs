...
public class LabourDataMigration : ControllerBase
{
    private readonly GENERICWriteOnlyCtx _writeOnlyCtx;
    private readonly GENERICWriteOnlyCtx _readOnlyCtx;

    public LabourDataMigration(GENERICWriteOnlyCtx writeOnlyCtx, GENERICWriteOnlyCtx readOnlyCtx)
    {
    ...
    [HttpPost("labourMigration")]
    public async Task<IActionResult> labourMigration(string filePath)
    {
        ...
        foreach (var row in dt.Rows.Cast<DataRow>())
        {
            ...
            if (!isExist)
            {
                ...
                var forest_range = DataRowHelper.GetStringValue(row, "Range");
                var forest_beat = DataRowHelper.GetStringValue(row, "Beat");
                ...
                otherMember.FullName = DataRowHelper.GetStringValue(row, "Name");
                otherMember.PhoneNumber = DataRowHelper.GetStringValue(row, "Mobile no");
                otherMember.NID = DataRowHelper.GetStringValue(row, "NID no");
                otherMember.MotherName = DataRowHelper.GetStringValue(row, "Mothers name");
                var isGenderExist = DataRowHelper.GetStringValue(row, "Gender");
