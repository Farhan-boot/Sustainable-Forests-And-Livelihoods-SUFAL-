...
public class MeetingMigrationV2 : ControllerBase
{
    private readonly GENERICWriteOnlyCtx _writeOnlyCtx;

    public MeetingMigrationV2(GENERICWriteOnlyCtx writeOnlyCtx)
    {
        _writeOnlyCtx = writeOnlyCtx;
    }

    [HttpPost("Meeting-Migration-V2")]
    public async Task<IActionResult> MigrateV2(string filePath, bool save = false)
    {
        ...
        foreach (var row in meetingDataTable.Rows.Cast<DataRow>().Skip(0))
        {
            ...
            if (string.IsNullOrEmpty(financial_year) == false)
            {
                financial_year = financial_year.Trim();

                var split = financial_year.Split("-");
                var first = split.First();
                var last = split.Last();

                var financial_year_db = await _writeOnlyCtx.Set<FinancialYear>()
                    .FromSqlRaw($"select * from \"GS\".\"FinancialYears\" e where e.\"StartYear\" = '{first}' and e.\"EndYear\" = '{last}'")
                    .FirstOrDefaultAsync();
        ...
        await System.IO.File.WriteAllTextAsync("C:\\Users\\PTSL-Jesy\\Desktop\\Bon\\problem-meeting.txt", System.Text.Json.JsonSerializer.Serialize(skippedMeetingListJsonFormat, new System.Text.Json.JsonSerializerOptions
        {
            WriteIndented = true,
        }));
